 ...
 */
public class SimulatorForMapParkingLane implements Simulator {
    ...
    public SimulatorForMapParkingLane(CPMMapParkingLane map){
        ...
        System.out.println("CPM Simulator for CPMMapParkingLane created!");
    }
    ...
    private List<Integer> cleanUpCompletedVehicles() {
        ...
        for(int vin : vinToVehicles.keySet()) {
            ...
            if(!vehicle.getShape().intersects(mapBoundary)) {
                ...
                assert map instanceof CPMBasicMap;
                ((CPMBasicMap)map).removeCompletedVehicle(vehicle);
                removedVINs.add(vin);
            }
        }
